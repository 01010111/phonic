...
Main.get_sound = function(src,title,icon,id) {
	...
	e.onclick = function() {
		...
		var tmp = "url(\"images/bg" + id % 4;
		return window.document.body.style.backgroundImage = tmp + ".png\")";
	};
...
Main.play = function(a,e) {
	...
	a.play().catch(function(e) {
		haxe_Log.trace(e,{ fileName : "src/Main.hx", lineNumber : 96, className : "Main", methodName : "play"});
...
util_HrefTools.fetch = function(href,then) {
	...
	request.onerror = function(e) {
		haxe_Log.trace(e,{ fileName : "src/util/HrefTools.hx", lineNumber : 10, className : "util.HrefTools", methodName : "fetch"});
	};
	request.send();
};
...
util_Pomodoro.init = function(v) {
	...
	util_Pomodoro.timer_text.onblur = function(e) {
		util_Pomodoro.on_blur(e);
	};
	util_Pomodoro.timer_text.addEventListener("input",function(e) {
		util_Pomodoro.on_input(e);
	});
	util_Pomodoro.play_btn = window.document.getElementById("pomo-start");
	util_Pomodoro.play_btn.onclick = function(e) {
		util_Pomodoro.play_btn_click(e);
	};
	util_Pomodoro.timer_icon = window.document.getElementById("pomo-icon");
	util_Pomodoro.timer_icon.onclick = function(e) {
...
zero_utilities_EventBus.dispatch = function(name,data) {
	if(!Object.prototype.hasOwnProperty.call(zero_utilities_EventBus.listeners.h,name)) {
		return;
	}
	var _g = 0;
	var _g1 = zero_utilities_EventBus.listeners.h[name];
	while(_g < _g1.length) {
		var listener = _g1[_g];
		++_g;
		if(listener != null && zero_utilities_EventBus.active_map.h[name]) {
			listener(data);
		}
	}
};
zero_utilities_EventBus.listen = function(listener,name) {
	if(!Object.prototype.hasOwnProperty.call(zero_utilities_EventBus.listeners.h,name)) {
		zero_utilities_EventBus.listeners.h[name] = [];
	}
	if(!Object.prototype.hasOwnProperty.call(zero_utilities_EventBus.active_map.h,name)) {
		zero_utilities_EventBus.active_map.h[name] = true;
	}
	zero_utilities_EventBus.listeners.h[name].push(listener);
};
zero_utilities_EventBus.unlisten = function(listener,name) {
	if(!Object.prototype.hasOwnProperty.call(zero_utilities_EventBus.listeners.h,name)) {
		return;
	}
	HxOverrides.remove(zero_utilities_EventBus.listeners.h[name],listener);
};
zero_utilities_EventBus.unlisten_all = function() {
	var key = haxe_ds_StringMap.keysIterator(zero_utilities_EventBus.listeners.h);
	while(key.hasNext()) {
		var key1 = key.next();
		zero_utilities_EventBus.listeners.h[key1] = [];
	}
};
...
zero_utilities_EventBus.set_active = function(name,active) {
	...
	zero_utilities_EventBus.active_map.h[name] = active;
};
